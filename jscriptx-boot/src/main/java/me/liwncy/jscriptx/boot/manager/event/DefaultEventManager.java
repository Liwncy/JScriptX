package me.liwncy.jscriptx.boot.manager.event;

import io.vertx.core.Future;
import lombok.NonNull;
import lombok.extern.slf4j.Slf4j;
import me.liwncy.jscriptx.core.manager.event.Event;
import me.liwncy.jscriptx.core.manager.event.EventListener;
import me.liwncy.jscriptx.core.manager.event.EventManager;
import me.liwncy.jscriptx.core.manager.event.PluginExceptionEvent;
    ...
    public Future<Void> publish(@NonNull Event<?> event) {
        ...
        return Future.future(promise -> {
            ...
            for (Plugin plugin : this.plugins) {
                ...
                // 检查事件源类型是否匹配监听器期望的事件源类型(范式会是null)
                if (listener.getSourceClass() != null && !listener.getSourceClass().isAssignableFrom(event.getData().getClass()))
                    continue;
