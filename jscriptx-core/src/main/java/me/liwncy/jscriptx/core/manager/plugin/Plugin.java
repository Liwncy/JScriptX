...
public abstract class Plugin implements PluginLifeCycle, Comparable<Plugin> {
    ...
    public final File getDataDir() {
        return new File(FileConstants.PLUGIN_DIR, this.description.getName());
    }
    ...
    public final void flushConfig() {
        var files = this.getDataDir().listFiles((dir, name) -> name.startsWith(Constants.CONFIG_JSON));
    ...
    public final void saveConfig() throws IOException {
        ...
        if (Objects.nonNull(files) && files.length > 0) {
            ...
            @Cleanup var is = Optional.ofNullable(
                    Optional.ofNullable(this.getClassLoader().getResourceAsStream(Constants.CONFIG_JSON5))
                            .orElse(this.getClassLoader().getResourceAsStream(Constants.CONFIG_JSON))
            ).orElseThrow(() -> new PluginException("无法找到默认配置文件"));
            this.vertx.fileSystem().mkdirsBlocking(this.getDataDir().getPath());
            this.vertx.fileSystem().writeFileBlocking(new File(this.getDataDir(), Constants.CONFIG_JSON5).getPath(), Buffer.buffer(is.readAllBytes()));
        }
    }
    ...
    public void saveDefaultConfig() {
        ...
        var file = Optional.ofNullable(this.getClassLoader().findResource(Constants.CONFIG_JSON5)).orElse(this.getClassLoader().findResource(Constants.CONFIG_JSON));
