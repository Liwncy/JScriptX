package me.liwncy.jscriptx.boot.manager.plugin;
...
public class DefaultPluginManager implements PluginManager {
    ...
    public Future<Void> onInit() throws Exception {
        // 1、读取插件配置
        return this.x.fileSystem().exists(FileConstants.PLUGIN_CONFIG_FILE.getPath())
                .compose(exists -> Future.future(promise -> {
                    if (!exists) {
                        // 如果配置文件不存在，则输出默认配置文件
                        var buffer = this.x.fileSystem().readFileBlocking(FileConstants.PLUGIN_CONFIG_FILE.getName());
                        this.x.fileSystem().writeFileBlocking(FileConstants.PLUGIN_CONFIG_FILE.getPath(), buffer);
                    }
                    this.x.fileSystem().readFile(FileConstants.PLUGIN_CONFIG_FILE.getPath())
...
//                                data.getJsonObject("limits").forEach(entry -> this.limits.put(ContactUtil.fromString(entry.getKey(), this.contactManager), ConvertUtil.toList(String.class, entry.getValue())));
                                ...
                                log.debug("插件配置文件读取成功，插件配置：{}",   this.config);
                                promise.complete();
                            });
                }))
    ...
    @Override
    public void reInit() {
        // this.tempData.getJsonObject("limits").forEach(entry -> this.limits.put(ContactUtil.fromString(entry.getKey()), Convert.toList(String.class, entry.getValue())));
        ...
        this.tempData = null;
        log.debug("插件配置重加载成功，插件配置：{}", this.config);
    }

    @Override
    public void loadPlugins() {
        // 1、遍历插件目录，获得插件文件
        var files = FileConstants.PLUGIN_DIR.listFiles((dir, name) -> name.endsWith("jar") || name.endsWith("zip"));
    ...
    public Future<Plugin> load(String name) {
        // 1、遍历插件目录
        var files = FileConstants.PLUGIN_DIR.listFiles((dir, fileName) -> fileName.endsWith("jar") || fileName.endsWith("zip"));
    ...
    @Override
    public Future<String> enable(String name) {
        return Future.future(promise -> {
            try {
                ...
                // }
                    ...
                    promise.complete(StrUtil.format("已启用插件 [{}]{}", name, tip));
                }
                this.saveConfig();
    ...
    @Override
    public Future<String> disable(String name) {
        return Future.future(promise -> {
            try {
                var plugin = this.get(name);
                // 如果target为空且插件已禁用
...
//                 }
                    ...
                    // }
                    var tip = plugin.isEnabled() ? "" : "(全局禁用)";
                    promise.complete(StrUtil.format("已禁用插件 [{}]{}", name, tip));
                }
                this.saveConfig();
            } catch (Exception e) {promise.fail(e);}
        });
    ...
    // @Override
    // public Collection<Plugin> availableList() {
    ...
    public Future<Void> saveConfig() {
        ...
        var buffer = Buffer.buffer(JsonObject.of("config", this.config).encodePrettily());
        return this.x.fileSystem().writeFile(FileConstants.PLUGIN_CONFIG_FILE.getPath(), buffer)
